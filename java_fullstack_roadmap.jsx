const javaWeeks = [
  {
    ...
    days: [
      { day: "Mon", hrs: 1.5, tasks: [
        ...
        "Understand primitive data types, wrapper classes, variables, operators, and formatting console output"
      ]},
      { day: "Wed", hrs: 1.5, tasks: [
        "Master Java flow control: if-else statements, switch cases, loops (for, while, do-while), and break/continue keywords"
      ]},
      ...
      { day: "Sun", hrs: 1.5, tasks: [
        "Master Exception Handling: try-catch-finally blocks, throw/throws keywords, checked vs unchecked exceptions, custom exceptions"
      ]}
    ]
  },
  {
    ...
    days: [
      ...
      { day: "Wed", hrs: 1.5, tasks: [
        "Learn inheritance models (super keyword, interfaces, abstract classes) and polymorphism (runtime vs compile-time)"
      ]},
      { day: "Fri", hrs: 1.5, tasks: [
      ...
      { day: "Sun", hrs: 1.5, tasks: [
        "Study Java Generics, Comparators vs Comparable, and functional programming with Java Streams API and Lambda expressions"
      ]}
    ]
  },
  {
    ...
    days: [
      ...
      { day: "Fri", hrs: 1.5, tasks: [
        "Master Spring configuration options: XML configuration, Java-based configuration (`@Configuration`, `@Bean`), annotation-based configuration (`@Component`, `@Autowired`)"
      ]},
      { day: "Sun", hrs: 1.5, tasks: [
        "Learn Spring Bean scopes (Singleton, Prototype, Request, Session) and bean lifecycle callbacks (`@PostConstruct`, `@PreDestroy`)"
      ]}
    ]
  },
  {
    ...
    days: [
      ...
      { day: "Wed", hrs: 1.5, tasks: [
        "Build REST API controllers: `@RestController`, `@RequestMapping`, `@GetMapping`, `@PostMapping`, `@PutMapping`, `@DeleteMapping`"
      ]},
      { day: "Fri", hrs: 1.5, tasks: [
        "Master request parameters extraction: `@PathVariable`, `@RequestParam`, `@RequestBody`, `@RequestHeader`, and JSON parsing"
      ]},
      { day: "Sun", hrs: 1.5, tasks: [
        "Implement REST API best practices: HTTP status codes, exception handling with `@ControllerAdvice` and `@ExceptionHandler`, DTO patterns"
      ]}
    ]
  },
  {
    ...
    days: [
      ...
      { day: "Wed", hrs: 1.5, tasks: [
        "Model Entity Relationships: `@OneToOne`, `@OneToMany`, `@ManyToOne`, `@ManyToMany` mappings, cascade types, lazy vs eager fetching"
      ]},
      { day: "Fri", hrs: 1.5, tasks: [
        "Master Spring Data JPA repositories: `JpaRepository` interface, custom query methods, `@Query` annotation (JPQL & Native SQL)"
      ]},
      { day: "Sun", hrs: 1.5, tasks: [
        "Learn database transactions: ACID properties, `@Transactional` configuration, transaction propagation and isolation levels"
      ]}
    ]
  },
  {
    ...
    days: [
      ...
      { day: "Wed", hrs: 1.5, tasks: [
        "Configure basic and form-based authentication, user roles and authorities, method-level security (`@PreAuthorize`)"
      ]},
      { day: "Fri", hrs: 1.5, tasks: [
        "Implement JSON Web Tokens (JWT) for stateless authentication: token generation, filter implementation, authorization checks"
      ]},
      { day: "Sun", hrs: 1.5, tasks: [
        "Handle CORS (Cross-Origin Resource Sharing), CSRF (Cross-Site Request Forgery) protections, and password hashing with BCrypt"
      ]}
    ]
  },
  {
    ...
    days: [
      ...
      { day: "Wed", hrs: 1.5, tasks: [
        "Implement Service Discovery using Netflix Eureka Server and Client discovery registration"
      ]},
      { day: "Fri", hrs: 1.5, tasks: [
        "Build an API Gateway using Spring Cloud Gateway to route client requests, handle CORS, and secure endpoints"
      ]},
      { day: "Sun", hrs: 1.5, tasks: [
        "Master Inter-service communication: synchronous REST calls via `RestTemplate` or `WebClient` and declarative REST clients with Spring Cloud OpenFeign"
      ]}
    ]
  },
  {
    ...
    days: [
      ...
      { day: "Wed", hrs: 1.5, tasks: [
        "Write integration tests using `@SpringBootTest` and configuring test profiles with H2 in-memory databases"
      ]},
      { day: "Fri", hrs: 1.5, tasks: [
        "Configure Spring Boot Actuator for health checks, metrics, and monitoring, alongside SLF4J/Logback configurations"
      ]},
      { day: "Sun", hrs: 1.5, tasks: [
        "Deploy Spring Boot application: Dockerize the application using multi-stage builds and deploy to AWS Elastic Beanstalk or Render"
      ]}
    ]
  }
];
